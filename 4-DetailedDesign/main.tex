...
\centering
\caption{N-point turn geometry} \figlabel{nPointTurnGeometry}
\end{figure} 
Throughout the N-point turn the quad bike must remain within the grey dashed lines else it would be traversing un-scanned terrain, risking the detonation of landmines. By analysing a 180 degree N-point turn in a 3 meter swathe, a template is constructed, and for any turn angles of less than 180 degrees the turn algorithm exits early, once the desired turn angle has been reached. The specific algorithms to achieve this are discussed further in \secref{pathsubdivision}. The template, shown in \tabref{npointTurnTemplate}, matches each point in the turn with an angle corresponding to the angle at which the quad bike touches the edge of the swathe. In \figref{nPointTurnGeometry}, the quad bike is at position (1, 26.7\degree). Accumulative error here is not an issue as this is the ideal path that the quad bike would follow, and any quad bike deviation error is corrected for by aiming for the ideal path point.
\begin{table} [ht]
\centering
...
$\Delta h$ (m) & 0 & 1.46 & 0.09 & 0.44 & 0.37 & 0.11 & 0.62 & -1.18 \\
\end{tabular}
\end{table}
...
For a simple turn, the algorithm to deduce the locations of the modified path is a fast, vector based, process. First, the centre of the turning circle is found by removing points from the subdivided straight line
...
When the quad bike is within a specified look-ahead distance from a goal point, the the point is incremented along the path and the process continues. This is true for all cases except when there is no next point where the path navigation is complete, or during N-point turn manoeuvres where complex navigation is required.
...
Hand calculations were used to verify the results from FEA. An axial force of 334 MPa was calculated for the rear frame section and this corresponded to a force of 333.99 MPa from FEA. 


The final design can be seen in  \figref{finalSensorFrameDesign} 
