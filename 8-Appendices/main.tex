...
FORESIGHT is a multi-sensor landmine detection system, mounted on a vehicle that can be controlled remotely. The vehicle usually travels with a protection vehicle, which clears the the path ahead of surface-based devices, as well as landmines just below the surface \parencite{canada2004}. The sensor suite consists of four independent systems, the data from which is fused to accurately detect a range of threats. A minimum metal detector array which is 3 m wide is used for the detection of metallic parts, even when the metal content is low. The 3 m GPR array with ultra-wide band capabilities (1-3 GHz) can detect larger landmines at up to 30 cm below the soil surface. An infra-red camera with 8-14 micrometre wavelength is used to detect changes in soil density which occur when landmines are buried \parencite{canada2004}. Finally, a thermal neutron activation detector directly detect explosives through measurements of the soil nitrogen level; this provides an added level of target confirmation \parencite{general2009}. The sensor suite can operate at a forwards speed of 3-4 km/h, and in the event that a detection is made with a sufficient confidence level, the onboard marking system is used to physically mark the threat location \parencite{canada2004}.
...
Three simulations were conducted, with the steering axle and sensor position changed in each. The results are shown in Figure E.1, and are discussed below.

\begin{figure}[ht]
\centerline{
\begin{tabular}{ccc}
\subfloat[Trial 1]{\includegraphics[width=0.3\textwidth]{3-ConceptDesign/Detector_Coverage1.png}}
& \subfloat[Trial 2]{\includegraphics[width=0.3\textwidth]{3-ConceptDesign/Detector_Coverage2.png}}
& \subfloat[Trial 3]{\includegraphics[width=0.3\textwidth]{3-ConceptDesign/Detector_Coverage3.png}}\\
\end{tabular}}
\caption*{\textbf{Figure E.1:} Sensor coverage simulations} 
\end{figure}
...
It is proposed that this level of control will be achieved through three independent (layered) systems to provide automation to the mobile platform, as shown in \Figref{auto_control}.
...
\caption[Input and output responsibilities of software subsystems]{Input and output responsibilities of software subsystems, showing data access boundaries.} \figlabel{auto_control}
\end{figure}
...
\subsubsection{Design and Implementation Constraints, and Operating Environment}

\nohyphens{	% Stop hyphenation in table
\begin{longtable}{ L{0.25\textwidth} L{0.65\textwidth}}
\caption[Subsystem outline]{Outline of software subsystems}\tablabel{implconstraints}\\ \toprule
\textbf{Subsystem} & \textbf{Hardware constraints} \\ \midrule\endfirsthead 
\caption[]{Outline of software subsystems (continued)}\\ \toprule
\textbf{Subsystem} & \textbf{Hardware constraints} \\  \midrule\endhead

% Heading &  ASCII SOH byte, indicating the start of a new transmission packet \\
% Packet ID & ID\_QUAD\_SPEED, indicating that this packet contains the current speed of the quad bike \\
% Packet Length & Packet contains a single float as data \\
% Data & An IEEE 754 floating point number, specifying the value 1.1f. This corresponds to a quad bike speed of 1.1 m/s. \\
% Terminus & Terminating character indicating the receipt of the full packet. \\ \midrule


Automation Control &
Must be capable of providing fast, quasi-real-time output to a series of I/O devices in order to control actuators. Direct access to I/O hardware will be required and operation on a system with minimal OS overhead and thread delay issues. Actual processing requirements are not computationally intensive.
 
Operation on a small, purpose-built microcontroller with direct I/O access will be ideal due to the requirement for signal output and low OS overhead. Under this arrangement, the software would likely be written in C/C++ with communications to other software systems provided by a USB/Serial communications layer. However, due to the ease of generating the required control systems using higher level equipment such as MATLAB and Simulink, this may be the preferred option to complete the requirements adequately in the project timeframe. \\ \midrule
...
\caption[Packet Structure]{Breakdown of the example packet}\tablabel{packetBreakdown}\\ \toprule
\textbf{Item} & \textbf{Hexadecimal} & \textbf{Interpretation}\\ \midrule\endfirsthead 
\caption[]{Stakeholders of the project (continued).}\\ \toprule
\textbf{Item} & \textbf{Hexadecimal} & \textbf{Interpretation}\\  \midrule\endhead

Heading & 0x01 & ASCII SOH byte, indicating the start of a new transmission packet \\
Packet ID & 0x52 & ID\_QUAD\_SPEED, indicating that this packet contains the current speed of the quad bike \\
Packet Length & 0x01 & Packet contains a single float as data \\
Data & 0x3F8CCCCD & An IEEE 754 floating point number, specifying the value 1.1f. This corresponds to a quad bike speed of 1.1 m/s. \\
Terminus & 0x17 & Terminating character indicating the receipt of the full packet. \\
